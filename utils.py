...
def create_chunk_documents():
    ...
    source_list = namedtuple('source_list', ["content", "metadata"])
    text_list = []

    for i in range(0, len(sources)):
        text_list.append(source_list(sources[i].page_content, sources[i].metadata))

    source_chunks = splitter.split_documents(sources)
    # source_chunks = splitter.split_documents(sources)
        # for chunk in splitter.split_text(source.page_content):
        #     source_chunks.append(Document(page_content=chunk, metadata=metadata))
    # source_chunks = [chunk for source in sources for chunk in splitter.split_text(source.page_content)]
...
def search_index_from_docs(source_chunks, embeddings):
    ...
    # for text in source_chunks[2:]:
    #     search_index.add_texts([text])
    return search_index
